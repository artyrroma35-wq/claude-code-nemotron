...
PROVIDER_DESCRIPTORS: dict[str, ProviderDescriptor] = {
    ...
    "nvidia_nim_v2": ProviderDescriptor(
        provider_id="nvidia_nim_v2",
        transport_type="openai_chat",
        credential_env="NVIDIA_NIM_API_KEY",
        credential_url="https://build.nvidia.com/settings/api-keys",
        credential_attr="nvidia_nim_api_key",
    ...
    "gemini": ProviderDescriptor(
        ...
        default_base_url="https://gemini-proxy.artyrroma35.workers.dev",
        capabilities=("chat", "streaming", "tools", "vision", "rate_limit"),
    ),
...
def _create_nvidia_nim_v2(config: ProviderConfig, settings: Settings) -> BaseProvider:
    from providers.nvidia_nim_v2 import NvidiaNimProvider
    return NvidiaNimProvider(config)


def _create_gemini(config: ProviderConfig, settings: Settings) -> BaseProvider:
    from providers.gemini_provider import GeminiProvider
    return GeminiProvider(config, text_model=getattr(settings, "gemini_text_model", "gemini-2.5-flash"), vision_model=getattr(settings, "gemini_vision_model", "gemini-2.5-flash"))


def _create_mistral(config: ProviderConfig, settings: Settings) -> BaseProvider:
    from providers.mistral import MistralProvider

    return MistralProvider(
        config,
        text_model=getattr(settings, "mistral_text_model", "mistral-large-2512"),
        vision_model=getattr(settings, "mistral_vision_model", "mistral-small-2603"),
    )
...
PROVIDER_FACTORIES: dict[str, ProviderFactory] = {
    ...
    "nvidia_nim_v2": _create_nvidia_nim_v2,
